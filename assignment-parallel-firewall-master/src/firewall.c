...
int main(int argc, char **argv)
{
	...
	fclose(context[0].log_file);
	pthread_mutex_destroy(context[0].log_mutex);
	pthread_cond_destroy(context[0].log_cond);
	free(context[0].log_mutex);
	free(context[0].log_cond);
	free(context[0].next_index_to_log);
	free(context[0].next_index_assigned);
